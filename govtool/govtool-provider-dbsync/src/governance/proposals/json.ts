/**
 * db-sync's `gov_action_proposal.description` is jsonb whose numbers can exceed
 * a float's exact range (lovelace) or carry decimal text a float would reshape.
 * `pg` parses jsonb with plain JSON.parse, so the provider selects it as text
 * and parses it here, keeping every number's source literal.
 */
