...
/**
 * Deterministic id for an anchor, hex:
 *
 *     blake2b-256( utf8(url) ‖ 0x23 ‖ bytes(dataHash) )
 *
 * where `bytes(dataHash)` is the 32-byte hash decoded from its hex form and
 * `0x23` is the literal `#` separator. Needed because the same dataHash can
 * appear at several urls, and the same url can serve different content over
 * time. Every producer must use exactly this construction or ids will not
 ...
 *
 * ../chain-data re-exports this; it is defined here because this service owns it.
 */
export interface MetadataProjection<TBody> {
  ...
  fetchedAt?: Timestamp;
  /**
   * The source's own failure text, when it records one and it does not map
   * onto `failureReason` — db-sync's `off_chain_vote_fetch_error.message` is
   * free text. Diagnostic; never the primary signal.
   */
  failureMessage?: string;
  /**
   * The unparsed document. Deliberately absent from the projection by default
   * (it is what `MetadataRecord.raw` is for); a provider includes it only when
   * its source hands it over in the same read and a consumer has asked.
   */
  raw?: unknown;
}
...
export interface MetadataServiceV1 {
  ...
  getMany(
    anchors: Anchor[],
    q?: MetadataQuery,
  ): Promise<Record<MetadataId, MetadataRecord>>;

  /** Raw stored document, unparsed. */
  getRaw(
    id: MetadataId,
  ): Promise<{ id: MetadataId; contentType?: string; raw: unknown }>;
