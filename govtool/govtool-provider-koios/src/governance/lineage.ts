...
export interface LineageLink {
  /** The action's CIP-129 id. */
  id: string;
  /** `previous_gov_action_proposal_id`; null at the start of the lineage. */
  prevId: string | null;
}

/**
 * Order enacted actions from the oldest to the enacted head. Every enacted
 * action must sit on the one chain: a fork or an orphan means the source
...
export function orderLineage<T extends LineageLink>(lineage: string, enacted: readonly T[]): T[] {
  ...
  for (;;) {
    ...
    if (!next) break;
    if (next.length > 1) {
      throw internal(`Koios records two enacted ${lineage} actions with the same predecessor`);
    }
    const link = next[0]!;
    ordered.push(link);
    prev = link.id;
  ...
  if (ordered.length !== enacted.length) {
    throw internal(`Koios records enacted ${lineage} actions that are not on the enacted chain`);
  }
  return ordered;
}
