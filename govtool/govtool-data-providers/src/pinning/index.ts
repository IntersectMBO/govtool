...
export interface PinRequest {
  ...
  content: string;
  contentType:
    'application/ld+json' | 'application/json' | 'text/plain' | string;
...
export type PinFailureReason =
  ...
  | 'BACKEND_UNAVAILABLE'
  | 'BACKEND_TIMEOUT'
  /** The backend answered with a failure status. `details` carries status and body. */
  | 'BACKEND_ERROR'
  /** The backend answered 2xx but the body was not what its API promises. */
  | 'BACKEND_INVALID_RESPONSE'
  /** This implementation does not offer the operation (mirrors chain-data's CAPABILITY_UNSUPPORTED). */
  | 'UNSUPPORTED_OPERATION';

export interface PinningErrorOptions {
  /** Whether retrying the same request could succeed. Defaults from the reason. */
  terminal?: boolean;
  /** Safe diagnostics — a backend's status code and response body, never its credentials. */
  details?: Record<string, unknown>;
  cause?: unknown;
}

const TERMINAL_BY_DEFAULT: ReadonlySet<PinFailureReason> =
  new Set<PinFailureReason>([
    'TOO_LARGE',
    'UNSUPPORTED_CONTENT_TYPE',
    'INVALID_METADATA',
    'QUOTA_EXCEEDED',
...
/**
 * The one error a pinning implementation throws across the contract boundary,
 * so a consumer maps `reason` onto its own transport without knowing which
 * backend is configured. Prefer `PinningError.is()` to `instanceof`; see
 * `ChainDataError` for why.
 */
export class PinningError extends Error {
  ...
  constructor(
    ...
    this.reason = reason;
    this.terminal = options.terminal ?? TERMINAL_BY_DEFAULT.has(reason);
  ...
  static is(value: unknown): value is PinningError {
    if (value instanceof PinningError) {
      return true;
    }
    ...
    return (
      candidate.name === 'PinningError' && typeof candidate.reason === 'string'
    );
  }
}
...
export interface PinningPolicy {
   ...
   * Optional: a thin adapter over a hosted backend does not know the plan's
   * limits and must not invent them.
   */
