...
WHERE
  (
    COALESCE($1, '') = '' OR
    (CASE WHEN LENGTH($2) % 2 = 0 AND $3 ~ '^[0-9a-fA-F]+$' THEN drep_hash = $4 ELSE false END) OR
    (CASE WHEN lower($5) ~ '^drep1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+$' THEN view = lower($6) ELSE FALSE END) OR
    given_name ILIKE $7
  )
