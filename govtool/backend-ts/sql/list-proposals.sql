...
WHERE
    (COALESCE($1, '') = '' OR
    off_chain_vote_gov_action_data.title ILIKE $2 OR
    off_chain_vote_gov_action_data.abstract ILIKE $3 OR
    off_chain_vote_gov_action_data.motivation ILIKE $4 OR
    off_chain_vote_gov_action_data.rationale ILIKE $5 OR
    concat(encode(creator_tx.hash, 'hex'), '#', gov_action_proposal.index) = $6)
GROUP BY
    gov_action_proposal.id,
    creator_tx.hash,
    creator_block.id,
