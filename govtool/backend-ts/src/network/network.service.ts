  ...
  async getNetworkTotalStake(): Promise<GetNetworkTotalStakeResponse> {
    return this.cacheService.getOrSet(
      ...
      async () => {
        ...
        const row = result.rows[0];
        return {
          totalStakeControlledByDReps: dbInteger(
            row.total_stake_controlled_by_active_dreps,
          ),
          totalStakeControlledBySPOs: dbInteger(
            row.total_stake_controlled_by_spos,
          ),
          alwaysAbstainVotingPower: dbInteger(row.always_abstain_voting_power),
          alwaysNoConfidenceVotingPower: dbInteger(
  ...
  async getNetworkMetrics(): Promise<GetNetworkMetricsResponse> {
    return this.cacheService.getOrSet('networkMetrics', 'default', async () => {
      ...
      return {
        ...
        totalRegisteredDirectVoters: this.toInteger(
