/**
 * The typed body of a governance action, decoded from db-sync's
 * `gov_action_proposal.description` — the ledger's own JSON rendering of the
 * action. SPEC.md §5.2: the body is required for all seven types, so an
 * undecodable description is an internal fault, never an untyped fallback.
 */
import type {
  GovActionBody,
  GovActionLineage,
...
import { asArray, asString, isObject, numText, parseExactJson, toPlain, type ExactJson } from './json';
import { toRatio } from './ratio';

/** db-sync `govactiontype` → contract type. The only rename is the committee one. */
 ...
 * committee lineage. Values are db-sync enum labels.
 */
...
export function toGovActionType(dbType: string): GovActionType {
  ...
  if (!type) throw internal(`Unknown db-sync governance action type ${dbType}`);
  return type;
}
...
/** Ledger JSON key → [contract key, conversion]. Keys absent from the contract keep their ledger name. */
...
const intOf = (v: ExactJson | undefined, what: string): number => {
  ...
  if (text === undefined || !/^-?\d+$/.test(text)) return fail(`${what} is not an integer`);
  return Number(text);
};
...
function convertParam(conv: Conv, v: ExactJson, key: string): unknown {
  switch (conv) {
    ...
    case 'costModels': {
      // Only the languages the action changes; each an integer array in ledger order.
      if (!isObject(v)) return fail(`${key} is not an object`);
      const models: Record<string, number[]> = {};
      for (const [language, costs] of Object.entries(v)) {
        const list = asArray(costs);
        if (!list) return fail(`${key}.${language} is not an array`);
        models[language] = list.map((c, i) => intOf(c, `${key}.${language}[${i}]`));
      }
      return models;
    }
    case 'version':
...
export function decodeBody(dbType: string, descriptionText: string, network: NetworkId): DecodedBody {
  const type = toGovActionType(dbType);
  if (type === 'InfoAction') return { body: { type } };
  if (type === 'NoConfidence') return { body: { type } };

  const description = parseExactJson(descriptionText);
