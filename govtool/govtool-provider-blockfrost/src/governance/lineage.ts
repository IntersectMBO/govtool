...
export interface LineageLink {
  ...
  id: string;
  /** The previous action's CIP-129 id; null at the start of the lineage. */
  prevId: string | null;
}
...
export function orderLineage<T extends LineageLink>(lineage: string, enacted: readonly T[]): T[] {
  ...
  for (;;) {
    ...
    if (!next) break;
    if (next.length > 1) {
      throw internal(`Blockfrost reports two enacted ${lineage} actions with the same predecessor`);
    }
    const link = next[0]!;
    ordered.push(link);
    prev = link.id;
  ...
  if (ordered.length !== enacted.length) {
    throw internal(`Blockfrost reports enacted ${lineage} actions that are not on the enacted chain`);
  }
  return ordered;
}
